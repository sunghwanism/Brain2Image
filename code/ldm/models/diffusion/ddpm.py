        ...
        with model.ema_scope():
            for count, item in enumerate(zip(data['fMRI'], data['Image'])):
                if limit is not None:
                    if count >= limit:
                        break
                latent = item[0].type('torch.FloatTensor') # fmri embedding
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        # self.num_timesteps = 1000 # 내가 임시로 넣은 것. 왜냐면 plms.py 15 line에서 model.num_timesteps를 불러오는 게 있는데 이상하게 멤버변수가 정의되어 있지 않음.

...
# own code
from config import Config_Generative_Model # config file 수정해서 넣어야됨*
from dataloader import NSD_Dataset # 따로 파일 만들어야됨*
from ldm.ldm_for_fmri import fLDM # 파일 추가*
from eval_metrics import get_similarity_metric # 파일 추가*
...
def to_image(img): 
    if img.shape[-1] != 3:
        img = rearrange(img, 'c h w -> h w c') # (3, 256, 256) -> (256, 256, 3)
    img = 255. * img
    return Image.fromarray(img.astype(np.uint8))

def channel_last(img): # (3, 256, 256) -> (256, 256, 3)
    if img.shape[-1] == 3:
        return img
    return rearrange(img, 'c h w -> h w c') 

def get_eval_metric(samples, avg = True): # sample -> generative_model.generate()의 리턴 값. return grid, (255. * torch.stack(all_samples, 0).cpu().numpy()).astype(np.uint8)
...
def fmri_transform(x, sparse_rate = 0.2): # x: (1, num_voxels), fmri의 일부를 랜덤하게 0으로 마스킹? 용도 모르겠음*
    x_aug = copy.deepcopy(x)
    idx = np.random.choice(x.shape[0], int(x.shape[0]*sparse_rate), replace = False)
    x_aug[idx] = 0
    return torch.FloatTensor(x_aug)

def main(config): # config: Config_Generative_Model + args
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    torch.manual_seed(config.seed)
    np.random.seed(config.seed)

    crop_pix = int(config.crop_ratio*config.img_size) # mind-vis에서는 crop_ratio = 0.2로 되어 있는데, 0.2*256 하면 51.2로 int 타입캐스트 하면 51됨. 이게 뭔지 확인해야됨*
    img_transform_train = transforms.Compose([
        normalize,
        random_crop(config.img_size - crop_pix, p = 0.5), # 256 - 51 = 205
        transforms.Resize((256, 256)), # 205, 205로 랜덤 크롭 한 후 다시 원래 사이즈로 복구
        channel_last # (3, 256, 256) -> (256, 256, 3)
    ])
    
    fmri_latents_dataset_train = NSD_Dataset(data_dir=config.data_dir, subject_num=config.subject_num, type='train')
    fmri_latents_dataset_test = NSD_Dataset(data_dir=config.data_dir, subject_num=config.subject_num, type='val_small')

    num_voxels = fmri_latents_dataset_train.num_voxels
    pretrain_mbm_metafile = torch.load(config.pretrain_mbm_path, map_location = 'cpu') # 태성이랑 혜원이한테 받아서 config 파일에 지정해야됨 *

    generative_model = fLDM(pretrain_mbm_metafile, num_voxels, # num_voxels에 뭐가 들어가는지 파악 *
                            device=device, pretrain_root=config.pretrain_gm_path, logger=config.logger,   # pretrin_gm_path: pretrains/ldm/label2img
                            ddim_steps=config.ddim_steps, global_pool=config.global_pool, use_time_cond=config.use_time_cond)
    
    if config.checkpoint_path is not None: # finetuning 여부 및 추가 학습 config 파일로 조절할 수 있음. 단 imagenet pretrained 모델 확인 해야됨 *
        model_meta = torch.load(config.checkpoint_path, map_location = 'cpu')
        generative_model.model.load_state_dict(model_meta['model_state_dict'])
        print('model resumed')
    # finetune the model
    trainer = create_trainer(config.num_epoch, config.precision, config.accumulate_grad, logger, check_val_every_n_epoch=50) # precision이랑 accumulate_grad가 뭘 의미하는지 모름, logger은 어디에 정의? *
    print("finetuning!!!!!!!!!!!")
...
if __name__ == '__main__':
    ...
    output_path = os.path.join('/NFS/Users/hwlee', 'results', 'generation',  '%s'%(datetime.datetime.now().strftime("%d-%m-%Y-%H-%M-%S")))
